...
| Key | Value | Required |
|-----|-------|----------|
| `NEXT_PUBLIC_API_URL` | `https://tulasiai.up.railway.app` | Yes |
| `NEXT_PUBLIC_BACKEND_URL` | `https://tulasiai.up.railway.app` | Yes |
...
## ✅ Completed Fixes
* **Memory Reduction**: Completely removed `chromadb` (which uses ~300MB RAM) and replaced it with `faiss-cpu` (under 10MB RAM). Railway free tier will no longer OOM.
* **API Standardization**: Backend now strictly checks for `GOOGLE_API_KEY` to run Gemini.
* **Dependency Fixes**: Pinned all versions to stop pip dependency-resolution timeouts on Railway.
...
* **Central API Client**: Created `@/lib/api.ts` so the frontend reliably talks to Railway. Connected the Chatbot UI to actually use the AI APIs.
