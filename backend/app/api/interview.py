...
async def answer_interview_audio(
    request: Request,
    session_id: str = Depends(lambda req: req.form().get("session_id")),
    answer: str = Depends(lambda req: req.form().get("answer", "")),
    audio_file: Optional[Any] = Depends(lambda req: req.form().get("audio_file")),
    db: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
):
    communication_tip = ""
    if audio_file:
        try:
            # Actually process audio using Gemini for real communication metrics
            import base64
            audio_bytes = await audio_file.read()
            # In a fully real implementation, this would be passed as a Part to google-genai
            # For now, without a direct file_api upload, we will extract text characteristics
            # from the provided `answer` (which is typically generated by STT on the frontend).
            # We explicitly REMOVE the fake emotion/confidence detection.
            
            # Since speech-to-text provides the text, we evaluate structural clarity and filler words.
            ...
            metrics = []
            if filler_count > 3:
                metrics.append(f"High filler word count ({filler_count} detected).")
            if word_count < 15:
                metrics.append("Response was very brief.")
            elif word_count > 150:
                metrics.append("Response was quite lengthy, consider being more concise.")
                
            if metrics:
                communication_tip = f" (Communication Metrics: {' '.join(metrics)})"
        except Exception as e:
            print(f"Audio processing error: {e}")

    return _process_answer(session_id, answer, db, current_user, communication_tip)
...
def answer_interview(
    ...
    return _process_answer(req.session_id, req.answer, db, current_user, "")

def _process_answer(session_id: str, answer: str, db: Session, current_user: User, comm_tip: str):
    query = select(PersistentInterviewSession).where(
        PersistentInterviewSession.session_id == session_id,
    ...
    full_answer = answer + (comm_tip if comm_tip else "")
...
Return ONLY valid JSON, no markdown."""
    ...
    # Get user profile information to make it completely personalized
    profile = current_user.profile
    profile_skills = profile.current_skills if profile else getattr(current_user, "skills", "")
    profile_skills_str = profile_skills if profile_skills else "No specific skills listed"
...
Candidate's Profile Skills: {profile_skills_str}
Note: Base your follow-up questions dynamically on the candidate's previous answer and explicitly test their stated skills where relevant.

Transcript so far:
{history_text}

Briefly acknowledge their last answer (1 short sentence), then ask question {interview_session.questions_asked} of {num_q}.
Make sure the question is a direct follow-up probing deeper into a technical decision they just mentioned, or testing a relevant skill from their profile.
