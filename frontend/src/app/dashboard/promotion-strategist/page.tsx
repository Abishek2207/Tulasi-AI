...
const PROMPTS = [
  ...
  { id: "1on1",       category: "1-on-1",      text: "How should I ask my manager what the exact expectations are for the next level?" },
  { id: "visibility", category: "Visibility",  text: "How do I make my work more visible to skip-level managers without sounding arrogant?" },
  { id: "selfreview", category: "Self Review", text: "Write a self-review highlighting my transition from IC to Tech Lead over the last 6 months." },
];
...
export default function PromotionStrategistPage() {
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", paddingBottom: 60 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "80px 32px", borderRadius: 24, background: "rgba(255,255,255,0.01)", border: "1px dashed rgba(255,255,255,0.08)", gap: 20 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.45)", lineHeight: 1.7, maxWidth: 380 }}>Complete your profile first so the Strategist can align to your career level.</p>
          </div>
          <Link href="/onboarding" style={{ padding: "12px 28px", borderRadius: 14, background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.3)", color: "#A78BFA", fontWeight: 700, fontSize: 14, textDecoration: "none" }}>Complete Profile →</Link>
        </div>
      </div>
    );
  }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ padding: 24, borderRadius: 24, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h3 style={{ fontSize: 15, fontWeight: 800, color: "white" }}>Strategy Scenario</h3>
          ...
          <div style={{ padding: 24, borderRadius: 24, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <textarea
              ...
              placeholder="Write your self-review, impact summary, or how you would frame the conversation..."
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 14 }}>
              ...
              <button
                ...
                style={{
                  ...
                  boxShadow: answer.trim() && !loading ? "0 8px 20px rgba(139,92,246,0.35)" : "none", transition: "all 0.2s",
                }}
              >
                {loading ? <><RefreshCw size={15} style={{ animation: "spin 1s linear infinite" }} /> Strategizing…</> : <><Send size={15} /> Evaluate Strategy</>}
              </button>
            </div>
          </div>
          ...
          <AnimatePresence>
            {feedback && (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                style={{ padding: 28, borderRadius: 24, background: "rgba(139,92,246,0.04)", border: "1px solid rgba(139,92,246,0.18)", display: "flex", flexDirection: "column", gap: 18 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", fontWeight: 700, marginTop: 2 }}>IMPACT SCORE</div>
                  </div>
                  <div style={{ flex: 1 }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: "rgba(255,255,255,0.6)", marginBottom: 10 }}>💡 Actionable Improvements</div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {feedback.suggestions.map((s, i) => (
                      <div key={i} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                        <CheckCircle2 size={15} color="#A78BFA" style={{ marginTop: 1, flexShrink: 0 }} />
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {avgScore !== null && (
            <div style={{ padding: 24, borderRadius: 20, background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.15)" }}>
              <div style={{ fontSize: 12, fontWeight: 800, color: "#A78BFA", marginBottom: 4, textTransform: "uppercase" }}>Avg Impact</div>
          ...
          <div style={{ padding: 20, borderRadius: 20, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)" }}>
            <h4 style={{ fontSize: 14, fontWeight: 800, color: "white", marginBottom: 14, display: "flex", alignItems: "center", gap: 8 }}><ArrowUpRight size={16} color="#8B5CF6" /> Trajectory</h4>
            {history.length === 0 ? <p style={{ fontSize: 13, color: "rgba(255,255,255,0.3)", textAlign: "center", padding: "20px 0" }}>Start planning to see history.</p> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {history.map((h, i) => (
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
            <h1 style={{ fontSize: 24, fontWeight: 900, color: "white" }}>Promotion Strategist</h1>
            <AgentBadge variant="beta" />
          </div>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>Impact docs · 1-on-1 framing · Corporate visibility</p>
        </div>
      </div>
