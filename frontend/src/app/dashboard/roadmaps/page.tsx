...
import { 
  ...
  Layout, Briefcase, GraduationCap, Code2, Compass, AlertCircle
} from "lucide-react";
...
export default function RoadmapsPage() {
  ...
  const fetchRoadmaps = async (isRetry = false) => {
    setLoading(true);
    if (!isRetry) setRoadmaps([]);
    setRetrying(false);
    try {
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto", paddingBottom: 100 }}>
      ...
      <AIResilienceWrapper
        loading={loading}
        retrying={retrying}
        result={roadmaps.length > 0 ? roadmaps : null}
        onRetry={() => fetchRoadmaps(true)}
        title="Neural Pathway Architect"
        subtitle="Synthesizing AGI-calibrated career trajectories..."
        color="#8B5CF6"
        icon={<BrainCircuit size={40} color="#8B5CF6" />}
      >
